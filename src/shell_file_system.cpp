...
namespace duckdb
{
																									 ...
																									 optional_ptr<FileOpener> opener)
	{
		...
		if (path.front() == '|')
		{
			...
			if (!pipe)
			{
				throw IOException({{"errno", std::to_string(errno)}}, "Could not open pipe for writing \"%s\": %s", path,
													strerror(errno));
			}
		...
		else
		{
			...
			if (!pipe)
			{
				throw IOException({{"errno", std::to_string(errno)}}, "Could not open pipe for reading \"%s\": %s", path,
													strerror(errno));
			}
